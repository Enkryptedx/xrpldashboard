    ...
    with pg_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                "  -- 2026-09-12 BEAR fix: AMM-issued LP tokens have "
                "  -- a 0x03-prefix currency by XLS-30 spec and always "
                "  -- fail ASCII decode, so they trip non_standard_code "
                "  -- by nature. They're not scams — they're pool-share "
                "  -- tokens. Excluding them stops labeled pools like "
                "  -- 'LP: XRP/BEAR' from spamming the warnings feed. "
                "  AND tv.currency NOT LIKE '03%%' "
