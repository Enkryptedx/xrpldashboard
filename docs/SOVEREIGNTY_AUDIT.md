...
| `credentials_state.py` (`/credentials`) | feature + account_objects/ledger_data type=credential + recent ledgers | `xrpl_client.XrplClient` → own node (`XRPL_LOCAL_NODE`); public Clio s2 → s1 fallback | own primary, public fallback | ✅ | disclosed third-party fallback (correct) — **GAP-3 closed 2026-09-25**. No LAN Clio exists: port 5006 is our rippled's JSON-RPC (probed: rippled build fields, no `clio_version`) and it answers every method the walker uses. One `walker_node_fallback` row per run (`RunFallbackSink`); `sourcing` persisted in the snapshot → `/credentials` banner; footer shows a node LABEL, not the raw LAN URL it used to print. Locked by `tests/test_credentials_sourcing.py` |
| `daily_snapshot.py`, `signed_snapshot.py` | snapshot metric reads | signed_snapshot: `_resolve_signing_node()` → own node (`XRPL_LOCAL_NODE`/LAN `XRPL_NODE`) as code default; s1 labeled fallback + `walker_node_fallback` event | own primary, public fallback | ✅ | **GAP-4 closed 2026-09-25** (`a18589f`) — each signed metric's `source` string names the ACTUAL node (`own-node (LAN) → …` vs `s1-fallback (…)`) |
| `rank_amms.py` (walker), `amm_scan_pools.py`, `scan_all_amms.py`, `amm_test.py` | AMM pool enumeration / amm_info | `xrpl_client.XrplClient` → own node (`XRPL_LOCAL_NODE`); `PUBLIC_NODES` s1 → s2 labeled fallback (scan_all_amms: s2 → s1, full-history first) | own primary, public fallback | ✅ | disclosed third-party fallback (correct) — **GAP-5 closed 2026-09-25**. One `walker_node_fallback` row per pass (`RunFallbackSink`); `sourcing` in the `amm_ranker` heartbeat extra + walker_health message → `/pools` banner. Side-finding fixed: `wallet_explainer._build_amm_lookup` was a LIVE s1 `amm_info` sweep from Render on every LP-holding `/wallet` render; now reads walker-cached `amm_ranked_pools`. `app.py` no longer imports the scan module's client/node. Locked by `tests/test_amm_ranking_sourcing.py` |
| `nft_activity_walker.py`, `scripts/nft_activity_gap_fill.py` | NFT activity (live forward-walker + 2026-04-01 history backfill) | live: `xrpl_client.XrplClient` → own node; `PUBLIC_NODES` labeled fallback. backfill/gap-fill: same client, labeled fallback list = public Clio archive `s2-clio` first (our node's window cannot hold 2026-04 history) | own primary, public fallback | ✅ live · third-party history (disclosed) | **GAP-6 closed 2026-09-25** — verified: NOT xrpscan. Live path was already own-node via `get_client`; the gap was one fallback row PER CALL (685k rows in the 09-19/20 catch-up) and a bare Clio client in backfill with no own-node attempt or sourcing flag. Now `RunFallbackSink` (one row per run) in every mode, `sourcing=` in walker_health, `/nfts` banner keyed off the forward-walker's last pass; backfill history stays labeled third-party in the source table. Locked by `tests/test_nft_activity_sourcing.py` |
...
| GAP-3 | `credentials_state.py` | Clio read on public s2 | **CLOSED 2026-09-25** — own-Clio confirm result: there is none; `192.168.40.95:5006` is rippled RPC. `XrplClient` own node primary, s2 (then s1) labeled fallback, one row per run, `/credentials` banner + label footer. `XRPL_CLIO_NODE` is no longer read by code. | done |
| GAP-4 | snapshot walkers | anchored metrics default-source public s1 | **CLOSED 2026-09-25** (`a18589f`) — `_resolve_signing_node()` own-node-first as CODE default, s1 labeled fallback with a `walker_node_fallback` event, every metric's `source` names the actual node. Proof run: 2026-09-25 21:00 ET leaf. | done |
| GAP-5 | AMM ranking scripts | hardcoded s1 | **CLOSED 2026-09-25** — `XrplClient` own node primary, labeled public fallback, one row per pass, `/pools` banner; Render-side live s1 sweep in the wallet explainer replaced by walker-cached rows. | done |
| GAP-6 | NFT activity walkers | source unverified (xrpscan?) | **CLOSED 2026-09-25** — verified own-node live path (not xrpscan); Clio archive only as the labeled backfill fallback; one row per run; `/nfts` banner. | done |

**All six gaps closed.** GAP-4 2026-09-25 (`a18589f`, signed_snapshot own-node-first as code default) · GAP-1 2026-09-25 (`681ccad`, network_pulse sourcing lock) · GAP-2 2026-09-25 (`f2c470b`, bridge_signer_walker) · GAP-3 2026-09-25 (`bc68d46`, credentials) · GAP-5 2026-09-25 (`496fe79`, AMM ranking + Render-side wallet-explainer sweep) · GAP-6 2026-09-25 (nft_activity). Every closed gap is locked by a hermetic sourcing test run in CI (`tests/test_*_sourcing.py`), so a re-opened gap turns CI red before Render deploys it (weekly claim-verify check item 3 is now mechanical for these surfaces).
