...
- **Day 6 rate limits + fleet-block extension + AI-crawler audit header (2026-08-02):** `agent_tier_rate_limit.py` new (~190 lines) — three classification functions (`is_ai_crawler`, `is_agent_tier_route`, `fleet_signature`) + one flask-limiter-callable rate function (`agent_tier_limit_rate`) + two constants (`AUDIT_URL_HEADER_NAME="X-XRPL-Dashboard-Audit-URL"`, `FLEET_BLOCK_RETRY_AFTER_SECONDS=86400`). `AI_CRAWLER_UA_SUBSTRINGS` is a positive allowlist (15 entries: gptbot, oai-searchbot, chatgpt-user, claudebot, claude-web, claude-searchbot, perplexitybot, perplexity-user, google-extended, googleother, applebot-extended, meta-searchbot, bytespider, ccbot, youbot) — public claim declared in agents.json rate_limits copy; additions require explicit review. `AGENT_TIER_ROUTE_PATHS` names the exact eight guarded paths (`/llms.txt`, `/.well-known/agents.json`, `/.well-known/security.txt`, `/robots.txt`, `/sitemap.xml`, `/.well-known/snapshots/chain.json`, `/.well-known/snapshots/pubkey.pem`, `/openapi.json`, `/docs`) plus `AGENT_TIER_ROUTE_PREFIXES=("/.well-known/snapshots/",)` for per-date snapshots — explicit not pattern-matched, so a new discovery surface is a review-worthy decision not an automatic broadening. `agent_tier_limit_rate()` returns `"300 per minute"` for AI crawlers, `"60 per minute"` otherwise, with env-var overrides `AGENT_TIER_AI_RATE`/`AGENT_TIER_ANON_RATE` for test-boundary exercises. `fleet_signature()` extends the /whales founding case (IL geo + Chrome/142 UA, first seen 2026-07-22, blocked 07-23 with no adaptation — IL /whales collapsed 210→30) to the agent-tier surface so the fleet can't rotate from /whales to citing our OpenAPI spec; returns the label `"IL_Chrome142_residential_2026_07"` (not just bool) so future fleets get distinguishable signatures. `app.py`: two hooks land after `_block_ai_crawlers` — `_agent_tier_fleet_block` (before_request, path-scoped via `is_agent_tier_route`, returns 429 + `Retry-After: 86400` + `X-Fleet-Signature: <label>` so instrumentation can see which block fired) and `_agent_tier_audit_header` (after_request, adds `X-XRPL-Dashboard-Audit-URL: {SITE_URL}/coverage` to AI-crawler responses on agent-tier routes only — the warm-citations touch, cold to anonymous scrapers). Eight route decorators added: `@limiter.limit(agent_tier_limit_rate)` above each `@app.route(...)` for the eight documented paths. flask-smorest `/openapi.json` + `/docs` are guarded by the global path-based fleet-block hook (their routes are registered by the Api instance, not `@app.route` — decoration is via the hook, not per-route limiter). `@app.errorhandler(429)` `_rate_limit_429` guarantees `Retry-After` on every 429 by extracting expiry from `RateLimitExceeded.limit.get_expiry()` with a 60s fallback — closes the "flask-limiter doesn't always set Retry-After" gap discovered mid-build. `_AGENTS_JSON.status.phase` bumped `Day 5 → Day 6`; `LAST_VERIFIED_AGENT_TIER_METHODOLOGY` bumped `2026-07-31 → 2026-08-02` (three-surface refresh: llms.txt, agents.json, /methodology#for-ai-agents). `templates/methodology.html`: rate-limits section note swapped "lands with the fleet-block extension in Day 6-7" copy for "enforcement is live as of 2026-08-02 … held in sync by a regression test". `CLAIMS.yaml:agents_json_rate_limits_match_enforcement` drops `needs-trace: true` (aspirational → satisfied) — risk_note updated to point at the module's default-rate strings as source of truth, at the eight `@limiter.limit` decorators as the wiring, and at `test_agents_json_rate_limit_copy_matches_module` as the regression guard. Twenty-three unit + integration tests in `tests/test_agent_tier_rate_limit.py`: `is_ai_crawler` classification (GPTBot / ClaudeBot case-insensitive / PerplexityBot / Google-Extended / regular Chrome false / empty-or-None false), `is_agent_tier_route` (exact set / snapshot-date prefix / human pages false / empty false), `fleet_signature` (IL+Chrome/142 matches / US+Chrome/142 no match — geo isolated / IL+Chrome/141 no match — version isolated / empty headers false), integration audit-URL header (present for AI crawler / absent for anonymous / absent even for AI crawler on human page — machine-consumption breadcrumb discipline), integration fleet-block (429 + Retry-After + X-Fleet-Signature label on `/llms.txt`; not applied to homepage — isolation from /whales inline block preserved), integration rate-limit boundary (pinch `AGENT_TIER_ANON_RATE` to `"2 per minute"`, 3rd request 429 + Retry-After — "never silent throttling" per §Rate limiting), AI-crawler higher bucket (anon pinched to 1/min while AI at 100/min, 3 requests as GPTBot all 200), fleet-block bypasses rate-limit bucket (ordering-matters test — fleet check fires before limiter), and `test_agents_json_rate_limit_copy_matches_module` (the sync guard: reads module defaults through the callable's default-branch by clearing env-var overrides in a `test_request_context`, asserts "60" appears in both module default AND agents.json anon copy, same for "300" and AI copy — the two stay wired at commit time). Full non-network suite: 96 + 23 = 119 tests, all green in 15.77s; 4 pre-existing failures on HEAD (`test_healthz_ok`, `test_public_page_returns_200[/health]`, `test_institutional_not_in_shared_nav`, `test_institutional_has_mailto_with_prefilled_subject`) verified unrelated via `git stash --include-untracked` re-run — Day 6 delta is +23 new, 0 regressions. Three-audience rule stays crisp: humans (never touched by this module — no agent-tier path is on the human surface), AI retrieval crawlers (never blocked, always identified, higher bucket + audit-URL header), scraper fleets (cost-defended — fleet check fires before bucket at request 1 of the burst). Fences pinned per module docstring: (1) route scope is explicit, not pattern-matched — extension requires review; (2) UA allowlist is a positive claim, additions AND removals both count; (3) fleet signature returns a label not a bool so `whales_cache_daily.blocked` can gain a signature dimension in the follow-up. Canary PASS this morning 06:00 EDT (trailing-7d delta=0, historical-week -5w delta=0, consecutive_failures=0) — Day 6 committed per standing law. Next: Day 7 = SHIP GATE (verification bar + Claude Desktop end-to-end demo ≥3 tools + screenshot + tool-call log to `docs/agent_tier_ship_evidence/`).
